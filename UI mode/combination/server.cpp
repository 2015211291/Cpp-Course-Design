...
server::server()
{
    ...
    // 连接信号槽,检测新的连接
    connect(server_i, &QTcpServer::newConnection, this, &server::server_New_Connect);
...
void server::socket_Read_Data()
{
    ...
    if(!buffer.isEmpty())
    {
        ...
        else
        {
            ...
            card_password = s_client.substr(index+1, index_2-index-1);
            pay_sum = s_client.substr(index_2+1);
