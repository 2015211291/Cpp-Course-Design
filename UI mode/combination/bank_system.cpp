...
/*type=4,转账;type=5,接受转账;type=6,购买商品扣钱*/
void Card::write_record(int type, double sum, string desti)
{
    string s;
...
string* Card::read_record(string s)
{
    ...
    for(int line_count=0; line_count<line_num; line_count++)
    {
        ...
        if((j == (one_record_line-1)) && (tmp[1] == s) && (tmp[0] == index))
            num++;
        cout << "num: " << num << endl;
    }
    f.close();
